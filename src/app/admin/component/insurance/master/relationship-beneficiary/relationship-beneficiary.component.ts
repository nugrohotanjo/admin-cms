...
import { RelationshipBeneficiaryService }                                    from './relationship-beneficiary.service'
...
@Component({
  selector: 'app-relationship-beneficiary',
  templateUrl: './relationship-beneficiary.component.html',
  styleUrls: ['./relationship-beneficiary.component.css']
})
export class RelationshipBeneficiaryComponent implements OnInit {
  ...
  constructor(private fb                          : FormBuilder,
              ...
              private relationshipBeneficiaryService : RelationshipBeneficiaryService,
  ...
  ngOnInit( ) {
    ...
    $(document).on('click', '#editRelationship', function(){
      let id = $(this).data('id');
      return self.relationshipBeneficiaryService.getRelationship2ById(id)
                        .subscribe((data) => {
    ...
    $(document).on('click', '#deleteRelationship', function(){
      let id = $(this).data('id');
        Swal.fire({
          ...
          return   self.relationshipBeneficiaryService.destroyRelationship2(id)
                    .subscribe(() => {
                      // save log
                      let user = self.getCurrentUserService.getUserData()
        
                      let message = "User " + user.name + " Delete Relationship Beneficiary" + self.relation_name.value

                      let category  = "Relationship Beneficiary"
                
                      const log = self.logService.storeLog(user, category, message).toPromise()

                      log.then(() => {
                        self.sweetalertService.yourWorkHasBeenSaved('Relationship beneficiary Has Been Delete')
  ...
  public createRelationship() {
    ...
    this.relationshipBeneficiaryService.saveRelationship2(this.relationshipForm.value)
                .subscribe((data) => {
                  // save log
                  let user = this.getCurrentUserService.getUserData()
                  ...
                  if(this.id.value)
                    message   = "User " + user.name + " Update Relationship Beneficiary " + this.relation_name.value
                  else 
                    message   = "User " + user.name + " Add new Relationship Beneficiary"
                  ...
                  log.then(() => {
                    this.sweetalertService.yourWorkHasBeenSaved('Relationship Beneficiary Has Been Save')
  ...
  public initDataTables(){
    ...
    $(document).ready(function() {
      $('#relationshipDatatables').DataTable({
        ajax: {
                'type'	      : 'GET',
                'url'	        : self.dataUrl +  '/list/customer-relation-beneficiary',
